...
export async function createMonthlyGardenIfNeeded(user) {
    ...
    const { monthKey: currentMonthKey } = computePeriodKeysFromDiaryContext(user.timezone, user.dayRolloverHour);
    ...
    console.log("[MONTHLY]", userId, "queued generate job for MONTH", lastCompletedMonthKey);
}
