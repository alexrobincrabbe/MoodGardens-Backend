...
function getIsoWeek(year, month, day) {
    ...
    const weekNumber = 1 +
        Math.round((thursday.getTime() - firstThursday.getTime()) /
            (7 * 24 * 60 * 60 * 1000));
...
function isoWeekToDate(weekYear, weekNumber) {
...
export function getWeekRangeFromWeekKey(weekKey) {
    ...
    const toDayKey = (d) => new Date(d.getTime()).toISOString().slice(0, 10); // "YYYY-MM-DD"
    return {
