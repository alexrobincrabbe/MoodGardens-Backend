...
export async function generateUserDEK(userId: string): Promise<{
  ...
  const plaintextKey = Buffer.from(resp.Plaintext as Uint8Array);       // 32 bytes
  const edek = Buffer.from(resp.CiphertextBlob as Uint8Array);          // opaque blob
 ...
 */
export async function decryptEDEK(
  edek: Uint8Array | Buffer,
  userId: string,
  keyId?: string
): Promise<Buffer> {
