...
async function backfillGardenSummaries() {
  ...
  console.log(
    `Gardens with plaintext summary but missing encryption fields: ${missingCount}`
  );
  ...
  for (;;) {
    ...
    for (const garden of gardens) {
      ...
      try {
        const encrypted = await encryptTextForUser(
          prisma,
          garden.userId,
          summary
        );
        ...
        console.error(
          `❌ Failed to encrypt summary for garden ${garden.id} (user ${garden.userId}):`,
          err
        );
      }
    }
  ...
  console.log(
    `✅ Garden summaries backfill complete. Total encrypted this run: ${processed}.`
  );
}
